...
def digits_to_words(input_string):
    ...
    * 만약 인풋 스트링에서 숫자가 존재하지 않는 다면, 빈 문자열 (empty string)을 반환함
    ...
    result = [code[int(x)] for x in input_string if x.isdigit()]
    return " ".join(result)
...
def to_camel_case(underscore_str):
    ...
    * 만약 어떤 변수 이름이 underscore로만 이루어 진다면, 빈 문자열만 반환해도 됨
    ...
    if len(underscore_str.strip("_")) == 0:
        return ""
    splited = underscore_str.strip("_").split("_")
    if len(splited) == 1:
        return underscore_str
    ret = "".join(str.capitalize() for str in splited if str.strip("_"))
